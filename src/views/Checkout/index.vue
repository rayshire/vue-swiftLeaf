...
const activeAddress = ref({
  ...
  address: "广东省温州市",
  isDefault: 1,
  fullLocation: "辽宁省 青海省 辽宁省",
})
const switchAddress = (item) => {
  activeAddress.value = item
}
function confirmAddress() {
  curAddress.value = JSON.parse(JSON.stringify(activeAddress.value))
  showDialog.value = false
  Object.assign(activeAddress.value, {});
}
...
async function getCheckoutInfo() {
  ...
  checkInfo.value = JSON.parse(JSON.stringify(res)).result
  curAddress.value = checkInfo.value.userAddresses.find(item => item.isDefault === 0)
  console.log("api", checkInfo.value)
}
onMounted(() => {
  getCheckoutInfo()
